...
main()
{
	...
	printf("1-Introduzca el tamanho de la sopa(matriz simetrica): ");
	scanf("%d", &n);

	while (n >= 41)
	{
		printf("\nPor motivos de espacio este juego no permite un tamanho mayor a 40\n");
		printf("\n1-Introduzca el tamanho de la sopa(matriz simetrica): ");
		scanf("%d", &n);
	}
	...
	for (i = 0; i < k; i++)
	{
		...
		while (repetir == 0)
		{
			...
			if (direccion == 0)
			{
				horizontal(sopa, palabras, largo, x, y, &repetir);
			}
			if (direccion == 1)
			{
				vertical(sopa, palabras, largo, x, y, &repetir);
			}
			if (direccion == 2)
			{
				diagonalp(sopa, palabras, largo, x, y, &repetir);
			}
			if (direccion == 3)
			{
				diagonals(sopa, palabras, largo, x, y, &repetir, n);
			}
		}
	}
...
int ncaracteres(char palabra[30])
{
	int count;
	count = 0;
